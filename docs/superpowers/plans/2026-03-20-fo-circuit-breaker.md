...
const existing = this.deps.db
  .prepare(`SELECT 1 FROM comms WHERE type = 'memo' AND mission_id = ? AND payload LIKE ? LIMIT 1`)
...
```typescript
const memos = rawDb
  .prepare("SELECT * FROM comms WHERE type IN ('memo', 'hailing-memo') ORDER BY created_at DESC")
...
it('dispatch() writes auto-escalation comm for persistent errors', () => {
  ...
  rawDb
    .prepare('UPDATE missions SET last_error_fingerprint = ? WHERE id = ?')
