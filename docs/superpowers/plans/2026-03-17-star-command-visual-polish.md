...
<CrtFrame>
  <StatusBar />
  <div className="flex-1 flex flex-col min-w-0">{/* existing header + content */}</div>
</CrtFrame>;
```
...
export class ShuttleAnimator {
  ...
  undock(crewId: string, podX: number, podY: number, canvasW: number, canvasH: number): void {
    ...
    if (edge < 0.25) {
      endX = -30;
      endY = Math.random() * canvasH;
    } else if (edge < 0.5) {
      endX = canvasW + 30;
      endY = Math.random() * canvasH;
    } else if (edge < 0.75) {
      endX = Math.random() * canvasW;
      endY = -30;
    } else {
      endX = Math.random() * canvasW;
      endY = canvasH + 30;
  ...
  update(deltaMs: number): void {
    ...
    this.anims = this.anims.filter((a) => a.elapsed - a.delay < a.duration);
  }
  ...
  render(ctx: CanvasRenderingContext2D): void {
    ...
    for (const anim of this.anims) {
      ...
      const eased = anim.type === 'dock' ? 1 - Math.pow(1 - t, 2) : t * t;
...
for (const pod of podStatesRef.current) {
  ...
  commsBeams.setPosition(
    pod.crewId,
    ringCX + Math.cos(midAngle) * podRadius,
    ringCY + Math.sin(midAngle) * podRadius
  );
}
```
...
import {
  ...
  isScSpriteReady,
  getScFrame
} from '../star-command/sc-sprite-loader';
import { SC_SPRITE_ATLAS } from '../star-command/sc-sprite-atlas';
```
