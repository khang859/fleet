...
<div class="fa-draw-tools">
  ...
  <button class="fa-tool-btn" data-tool="line" title="Line / Arrow (L, hold Shift for arrow)">
    \u2571</button
...
function onCanvasMouseUp(e) {
  ...
  if (currentDrawOp.type === 'line') {
    var ld = Math.hypot(
      currentDrawOp.end[0] - currentDrawOp.start[0],
      currentDrawOp.end[1] - currentDrawOp.start[1]
    );
    if (ld < 3) dominated = true;
  }
  if (currentDrawOp.type === 'rect') {
    if (Math.abs(currentDrawOp.size[0]) < 3 && Math.abs(currentDrawOp.size[1]) < 3)
      dominated = true;
  }
  if (currentDrawOp.type === 'ellipse') {
    if (Math.abs(currentDrawOp.radii[0]) < 2 && Math.abs(currentDrawOp.radii[1]) < 2)
      dominated = true;
  }
...
function toggleShape() {
  ...
  if (shapeBtn) {
    ...
    shapeBtn.title =
      activeShape === 'rect'
        ? 'Rectangle (S, click again for ellipse)'
        : 'Ellipse (S, click again for rectangle)';
  }
}
```
...
function onKeyDown(e) {
  ...
  if (!e.ctrlKey && !e.metaKey) {
    if (e.key === 'p' || e.key === 'P') {
      e.preventDefault();
      setActiveTool(activeTool === 'pen' ? 'pick' : 'pen');
      return;
    }
    if (e.key === 'l' || e.key === 'L') {
      e.preventDefault();
      setActiveTool(activeTool === 'line' ? 'pick' : 'line');
      return;
    }
    if (e.key === 's' || e.key === 'S') {
      e.preventDefault();
    ...
    if (e.key === 't' || e.key === 'T') {
      e.preventDefault();
      setActiveTool(activeTool === 'text' ? 'pick' : 'text');
      return;
    }
  }
...
if (preCapture) {
  ...
  if ((result as any).canvasOverlay) {
    const composited = await this.compositeOverlay(
      preCapture,
      (result as any).canvasOverlay,
      viewport
    );
