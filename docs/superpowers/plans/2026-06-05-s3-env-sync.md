...
**Architecture:** A committed, secret-free `.fleet/env-sync.json` pointer describes per-repo S3 addressing and one or more env-file `targets[]`. A main-process `env-sync` module (six focused units) encrypts/decrypts with AES-256-GCM + scrypt, talks to S3 via the AWS SDK default credential chain, detects conflicts by ETag, and either writes `.env` files or injects vars into Fleet terminals. The renderer gets a Settings section, a workspace status badge, and a conflict-diff dialog.
...
- `env-sync-secrets.ts` — keychain-backed global + per-repo passphrases (redacted views).
- `s3-client.ts` — `head`/`get`/`put` wrapper over `@aws-sdk/client-s3`.
...
/** Safe-for-IPC view of stored passphrases. */
...
describe('EnvSyncSecrets', () => {
  it('redacts: presence flags only, never plaintext', () => {
    ...
    expect(r).toEqual({ globalPresent: true, repoOverrides: { app: { present: true } } });
  });
...
import type { RedactedEnvSyncSecrets } from '../../shared/env-sync-types';
...
export class EnvSyncSecrets {
  ...
  getRedacted(): RedactedEnvSyncSecrets {
    ...
    return { globalPresent: Boolean(raw.globalPassphraseEnc), repoOverrides };
  }
  ...
  private assertEncryption(): void {
    if (!this.safe.isEncryptionAvailable()) {
      throw new Error('OS keychain encryption is unavailable; cannot store passphrase.');
    }
  }
}
```
...
Run: `npx vitest run src/main/__tests__/env-sync-secrets.test.ts`
Expected: PASS (5 tests). Note: the fake `safeStorage` returns a marker buffer; `resolvePassphrase` reads `Buffer.from(enc, 'base64')` — the fake encode also base64s, so decode round-trips.

- [ ] **Step 5: Commit**

```bash
git add src/main/env-sync/env-sync-secrets.ts src/main/__tests__/env-sync-secrets.test.ts
git commit -m "feat(env-sync): add keychain-backed passphrase secrets manager"
```
...
No unit test — this is a thin SDK adapter verified via the manager (mocked) and end-to-end manually. Keep it minimal.
...
function client(region: string): S3Client {
  ...
  if (!c) {
    c = new S3Client({ region, credentials: fromNodeProviderChain() });
...
export async function head(bucket: string, region: string, key: string): Promise<S3Head> {
  try {
    const r = await client(region).send(new HeadObjectCommand({ Bucket: bucket, Key: key }));
    return { etag: r.ETag ?? '' };
  } catch (err) {
...
export async function get(bucket: string, region: string, key: string): Promise<S3Get> {
  const r = await client(region).send(new GetObjectCommand({ Bucket: bucket, Key: key }));
...
export async function put(
  ...
  const r = await client(region).send(
...
```bash
git add package.json package-lock.json src/main/env-sync/s3-client.ts
git commit -m "feat(env-sync): add S3 client wrapper and AWS SDK deps"
```
...
function fakeS3() {
  ...
  return {
    objs,
    head: async (b: string, _r: string, k: string) => objs.get(`${b}/${k}`) ? { etag: objs.get(`${b}/${k}`)!.etag } : null,
    get: async (b: string, _r: string, k: string) => { const o = objs.get(`${b}/${k}`)!; return { body: o.body, etag: o.etag }; },
    put: async (b: string, _r: string, k: string, body: Buffer, ifMatch?: string) => {
...
const fakeSecrets = { resolvePassphrase: () => 'pw' };
...
import type { EnvSyncConfig, EnvSyncTarget, TargetStatus, SyncOutcome } from '../../shared/env-sync-types';
...
export class EnvSyncManager {
  ...
  async status(repoDir: string): Promise<TargetStatus[]> {
    ...
    for (const target of config.targets) {
      ...
      try {
        const remote = await this.s3.head(bucket, region, objectKey);
  ...
  async pull(
    ...
    try {
      ...
      const remote = await this.s3.get(bucket, region, objectKey);
  ...
  async push(
    ...
    try {
      ...
      const ifMatch = opts.force ? await this.currentEtag(bucket, region, objectKey) : state?.lastEtag;
      const result = await this.s3.put(bucket, region, objectKey, blob, ifMatch);
      this.setState(repoDir, objectKey, {
  ...
  private async currentEtag(bucket: string, region: string, objectKey: string): Promise<string | undefined> {
    const h = await this.s3.head(bucket, region, objectKey);
    return h?.etag;
  }
  ...
  private async safeRemoteText(bucket: string, region: string, objectKey: string, id: string): Promise<string | null> {
    try {
      ...
      const r = await this.s3.get(bucket, region, objectKey);
  ...
  async getEnvForCwd(cwd: string): Promise<Record<string, string>> {
    ...
    try {
      ...
      const obj = await this.s3.get(bucket, region, objectKey);
...
import type { EnvSyncSetPassphraseRequest, EnvSyncClearPassphraseRequest } from '../shared/ipc-api';
import { safeStorage } from 'electron';
```
...
export function EnvSyncSection(): JSX.Element {
  ...
  const [secrets, setSecrets] = useState<RedactedEnvSyncSecrets>({ globalPresent: false, repoOverrides: {} });
...
Note: the "Scan to add targets", per-repo passphrase override, and bucket/region editing reuse the same `window.fleet.envSync.scan` / `writeConfig` / `setPassphrase` calls; add them as follow-up rows in this component once the core flow is verified. The conflict path dispatches a window event consumed by Task 11.
...
Run:
```bash
npm run typecheck && npm run lint && npx vitest run src/main/__tests__/env-sync-crypto.test.ts src/main/__tests__/env-file.test.ts src/main/__tests__/env-sync-config.test.ts src/main/__tests__/env-sync-secrets.test.ts src/main/__tests__/env-sync-manager.test.ts src/shared/__tests__/env-sync-types.test.ts
```
...
4. Settings → Env Sync → set a global passphrase.
...
- **Spec coverage:** committed config (T1/T4), monorepo targets + scan (T1/T3/T10), nearest-ancestor + most-specific inject (T4/T7/T9), crypto scheme incl. `maxmem` (T2), keychain passphrases global+repo (T5), AWS chain via S3 wrapper (T6), bidirectional sync + ETag conflict + diff dialog (T7/T11), file & inject delivery (T7/T9), Settings UI + badge + dialog (T10/T11). All spec sections map to tasks.
- **Type consistency:** `SyncOutcome`, `TargetStatus['state']`, `EnvDiff`, `RedactedEnvSyncSecrets`, `ConflictChoice`, `DiscoveredRepo` defined once in T1 and reused verbatim in T7–T11. Manager method names (`status`/`pull`/`push`/`resolveConflict`/`diff`/`getEnvForCwd`) match between T7 implementation and T8 handlers.
