...
- **MAJOR** — 409 vs 412 corrected (see research-validation pass below): 412 is the divergence conflict (diff prompt); 409 is a transient race reported as a retryable error, never folded into `isPreconditionFailed`.
...
## Security research-validation pass (addressed)

Validated against reputable primary sources (OWASP Cheat Sheets, NIST SP 800-38D, AWS S3 docs, Electron docs via Context7) on 2026-06-05.

- **scrypt params — CONFIRMED.** OWASP Password Storage Cheat Sheet: "use scrypt with a minimum CPU/memory cost parameter of (2^17), a minimum block size of 8 (1024 bytes), and a parallelization parameter of 1." Our `N=2¹⁷, r=8, p=1` is an exact match.
- **AES-256-GCM — CONFIRMED.** OWASP Cryptographic Storage Cheat Sheet: AES with ≥128-bit (ideally 256-bit) key + an authenticated mode (GCM preferred). Matches our scheme.
- **96-bit random IV — CONFIRMED safe for our usage.** NIST SP 800-38D limits a random 96-bit IV to 2³² invocations *per key*. We derive a **fresh scrypt key (fresh 16-byte salt) on every encryption**, so each key encrypts exactly once — the birthday bound is never approached. No change needed; documented here so the fresh-salt-per-encryption invariant is treated as load-bearing, not incidental.
- **S3 conditional-write error model — BUG FOUND & FIXED.** AWS "How to prevent object overwrites with conditional writes" is explicit that for `If-Match`, **412 Precondition Failed** = ETag diverged / first-writer-wins (a real conflict), whereas **409 Conflict** = transient concurrent-request race ("uploads may be retried after receiving a 409 Conflict error"). The plan had folded 409 into `isPreconditionFailed`, which would surface a transient race as a misleading keep-local/keep-remote diff. Fixed: `isPreconditionFailed` is now 412-only, new `isConditionalConflict` (409) reports a retryable error, with tests (T6) and the `S3Like`/fake updated (T7).
- **`If-Match` IAM (`s3:PutObject` + `s3:GetObject`) — CONFIRMED.** AWS docs: "To perform conditional writes with an HTTP `If-Match` header you must have the `s3:PutObject` and `s3:GetObject` permissions." The IAM note in T12 is correct.
- **Electron `safeStorage` — CONFIRMED, with a recommendation.** Context7 (Electron docs) confirms the Linux hardcoded-plaintext-password fallback (our `basic_text` warning is right) and the macOS Keychain / Windows DPAPI backing. Electron now **recommends the async API** (`encryptStringAsync`/`decryptStringAsync`) over the sync API (which "may be deprecated in a future version"). We keep the **sync** API in v1 to mirror the existing `PiEnvInjectionManager` pattern (safeStorage sync calls hit the OS keychain and are fast — unlike scrypt, which is why only scrypt was made async). **Follow-up:** migrate both `PiEnvInjectionManager` and `env-sync-secrets` to the async safeStorage API together.
- **404-on-`If-Match`-PUT (concurrent delete) — noted, not coded.** AWS docs say a concurrent delete during an `If-Match` PUT yields 404 ("reupload the object"). v1 has no delete operation, so this can only occur via an out-of-band console delete mid-push; it falls through to the generic error path. Intentionally not handled (YAGNI) — listed so it isn't mistaken for a gap.
