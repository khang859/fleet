...
Each automation is a JSON file in `~/.fleet/automations/`:
...
Each automation gets a sibling directory for run outputs:

```
~/.fleet/automations/
  deploy-staging.json              <- automation config
  deploy-staging/                  <- output directory
    2026-03-25T09-00-00/           <- per-run folder (timestamp)
      report.csv
      summary.md
    2026-03-25T10-00-00/
      report.csv
...
API keys configured in Fleet settings, not stored in automation JSON.
...
- Subtle status indicator per item: idle, running, error
- New tab type added: `'terminal' | 'star-command' | 'crew' | 'file' | 'image' | 'automation'`
...
- Run button: starts the automation
...
- Schedule presets dropdown: Every 5 min, Every 15 min, Every hour, Every 6 hours, Daily at 9am, Daily at midnight, Weekdays at 9am, Weekly Monday 9am, Monthly 1st at 9am, First of month midnight
...
- Max steps: number input, default 25
- Max tokens: number input, default 8192
...
2. Main process loads automation JSON from disk
3. Creates a timestamped output directory for this run
4. Calls Vercel AI SDK `generateText()` with configured provider, model, prompt, tools, maxSteps
5. Agent system prompt includes: user's system prompt + output directory path + available tool descriptions
6. Agent loops: generate → tool call → execute → feed result back → until done or maxSteps hit
7. Logs stream to renderer via IPC in real time
8. On completion: status updated (success/error), output directory scanned for files, UI refreshed

### Tool implementations

| Tool | Implementation |
|------|---------------|
| `shell` | Spawns child process via `child_process.exec`. Returns stdout/stderr. |
| `read_file` | `fs.readFile()`. Returns file contents as string. |
| `write_file` | `fs.writeFile()`. Scoped to output directory by default but can write elsewhere. |
| `fleet` | Calls `invokeFleetCli()` (existing). Returns JSON response. |

### Cron scheduling

- `node-cron` in the main process (lightweight, no external deps)
- On app launch: reads all automation files, registers cron jobs for schedule triggers
- On automation save: re-registers that automation's cron job
...
interface AutomationStore {
  ...
  // Execution
  runningAutomations: Map<string, RunState>
...
|---------|------|---------|
| `AUTOMATION_LIST` | handle | List all automation files |
| `AUTOMATION_READ` | handle | Read a single automation by ID |
| `AUTOMATION_WRITE` | handle | Create or update an automation |
| `AUTOMATION_DELETE` | handle | Delete an automation file |
| `AUTOMATION_RUN` | handle | Start execution |
| `AUTOMATION_STOP` | on | Cancel running automation |
| `AUTOMATION_LOG` | streaming | Log events from main → renderer |
| `AUTOMATION_OUTPUTS` | handle | List output files for a run |
...
- By default scoped to the run's output directory
- Can write to any path (the agent is trusted at the same level as the user)
...
- Calls `invokeFleetCli()` internally (no socket overhead)
...
- **Run history persistence** — track past run statuses beyond just output files
