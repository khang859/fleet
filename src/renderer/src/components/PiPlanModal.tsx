...
export function PiPlanModal({
  ...
  // Hold the last plan so its content stays rendered through the exit animation,
  // after the parent has dequeued it and `plan` has gone null.
  const [shownPlan, setShownPlan] = useState(plan);
  useEffect(() => {
    if (plan) setShownPlan(plan);
  }, [plan]);
  const displayed = plan ?? shownPlan;

  const filePath = displayed?.path ?? null;
  const canRespond = Boolean(displayed?.paneId && displayed.requestId);
      ...
      {filePath && (
        ...
        >
          <div className="flex items-center justify-between px-4 py-2 border-b border-neutral-800 shrink-0 bg-neutral-950/90">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                className="inline-flex items-center gap-1.5 px-2 py-1 text-xs text-neutral-400 hover:text-white rounded transition hover:bg-neutral-800 active:scale-[0.97]"
              >
          ...
          <div className="flex-1 min-h-0 overflow-hidden">
            <MarkdownPane
              key={`${filePath}:${displayed?.requestId ?? contentKey ?? ''}`}
              ...
              <>
                {submitError && (
                  <div className="flex items-center justify-between gap-3 rounded border border-red-900/60 bg-red-950/30 px-2 py-1.5 text-xs text-red-300">
                    ...
                    <button
                      onClick={onClose}
                      className="shrink-0 rounded px-2 py-0.5 text-red-200 transition hover:bg-red-900/40 active:scale-[0.97]"
                    >
                      Dismiss
                    </button>
                  </div>
                ...
                <div className="flex justify-end gap-2">
                  <button
                    ...
                    className="px-3 py-1.5 text-sm rounded border border-red-900/60 text-red-300 transition hover:bg-red-950/40 active:scale-[0.97] disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100"
                  >
                  ...
                  <button
                    ...
                    disabled={submittingAction !== null}
                    className="px-3 py-1.5 text-sm rounded border border-neutral-700 text-neutral-300 transition hover:bg-neutral-800 active:scale-[0.97] disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100"
                  >
                  ...
                  <button
                    ...
                    disabled={submittingAction !== null}
                    className="px-3 py-1.5 text-sm rounded bg-teal-600 text-white transition hover:bg-teal-500 active:scale-[0.97] disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100"
                  >
