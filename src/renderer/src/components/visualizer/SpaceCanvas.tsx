...
function workspaceToAgents(
  tabs: { id: string; label: string; splitRoot: import('../../../../shared/types').PaneNode }[]
): AgentVisualState[] {
  return tabs.map((tab) => {
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (hit) {
      ...
      if (ship) {
        ...
        const paneCount = ship.isSubAgent
          ? undefined
        ...
        setTooltip({
          ...
          y: tooltipY,
          label: ship.label,
          panes: ship.isSubAgent
            ? 'pane'
            : `${(paneCount ?? 0) + 1} pane${(paneCount ?? 0) + 1 !== 1 ? 's' : ''}`
        })
        return
      }
    }
