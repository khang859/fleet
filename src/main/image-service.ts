...
export class ImageService extends EventEmitter {
  ...
  async generate(opts: {
    ...
    const meta: ImageGenerationMeta = {
      ...
      provider: provider.id, model, mode: 'generate',
      prompt: opts.prompt,
      params: { resolution, aspect_ratio: aspectRatio, output_format: outputFormat, num_images: opts.numImages ?? 1 },
      referenceImages: [], images: [], providerRequestId: null
    };
    this.writeMeta(id, meta);

    const genOpts: GenerateOpts = { model, prompt: opts.prompt, resolution, aspectRatio, outputFormat, numImages: opts.numImages };
  ...
  async edit(opts: {
    ...
    const meta: ImageGenerationMeta = {
      ...
      createdAt: new Date().toISOString(),
      completedAt: null, failedAt: null, error: null,
      provider: provider.id, model, mode: 'edit',
      prompt: opts.prompt,
      params: { resolution, aspect_ratio: aspectRatio, output_format: outputFormat, num_images: opts.numImages ?? 1 },
      referenceImages: opts.images, images: [], providerRequestId: null
    };
    this.writeMeta(id, meta);

    const editOpts: EditOpts = { model, prompt: opts.prompt, imageUrls: opts.images, resolution, aspectRatio, outputFormat, numImages: opts.numImages };
  ...
  getStatus(id: string): ImageGenerationMeta | null { return this.readMeta(id); }
  ...
  async retry(id: string): Promise<{ id: string }> {
    ...
    meta.status = 'queued'; meta.error = null; meta.failedAt = null; meta.completedAt = null; meta.images = []; meta.providerRequestId = null;
    this.writeMeta(id, meta);

    const isEdit = meta.mode === 'edit';
    const opts = isEdit
      ? { model: meta.model, prompt: meta.prompt, imageUrls: meta.referenceImages, resolution: meta.params.resolution, aspectRatio: meta.params.aspect_ratio, outputFormat: meta.params.output_format, numImages: meta.params.num_images } satisfies EditOpts
      : { model: meta.model, prompt: meta.prompt, resolution: meta.params.resolution, aspectRatio: meta.params.aspect_ratio, outputFormat: meta.params.output_format, numImages: meta.params.num_images } satisfies GenerateOpts;
  ...
  private submitAndPoll(id: string, provider: ImageProvider, opts: GenerateOpts | EditOpts): void {
    void (async () => {
      try {
        ...
        const meta = this.readMeta(id);
        if (meta) { meta.providerRequestId = requestId; meta.status = 'processing'; this.writeMeta(id, meta); this.emit('changed', id); }
  ...
  private pollLoop(id: string, provider: ImageProvider, requestId: string): void {
    ...
    void (async () => {
      ...
      while (!controller.signal.aborted) {
        if (Date.now() - startTime > POLL_TIMEOUT_MS) { this.markStatus(id, 'timeout', 'Generation timed out after 300s'); break; }
        ...
        try {
          ...
          if (poll.status === 'completed') { await this.downloadResults(id, provider, requestId); break; }
          else if (poll.status === 'failed') { this.markFailed(id, poll.error); break; }
        } catch (err) {
          const msg = err instanceof Error ? err.message : String(err);
          if (msg.includes('401') || msg.includes('Unauthorized')) { this.markFailed(id, 'Invalid API key. Run: fleet images config --api-key <key>'); break; }
          if (msg.includes('429') || msg.includes('rate limit')) { this.markFailed(id, 'Rate limited by provider. Try again later.'); break; }
  ...
  private async downloadResults(id: string, provider: ImageProvider, requestId: string): Promise<void> {
    try {
      ...
      for (let i = 0; i < result.images.length; i++) {
        ...
        try {
          ...
          hasFailure = true;
          imageEntries.push({ filename: null, width: img.width, height: img.height, error: `Download failed: ${dlErr instanceof Error ? dlErr.message : String(dlErr)}`, providerUrl: img.url });
        }
      }

      meta.images = imageEntries;
      if (hasFailure && imageEntries.some((e) => e.filename != null)) { meta.status = 'partial'; }
      else if (hasFailure) { meta.status = 'failed'; meta.error = 'All image downloads failed'; meta.failedAt = new Date().toISOString(); }
      else { meta.status = 'completed'; meta.completedAt = new Date().toISOString(); }

      this.writeMeta(id, meta);
      this.emit('changed', id);
    } catch (err) { this.markFailed(id, `Download failed: ${err instanceof Error ? err.message : String(err)}`); }
  }
  ...
  private markFailed(id: string, error: string): void { this.markStatus(id, 'failed', error); }

  private markStatus(id: string, status: ImageGenerationStatus, error: string): void {
    const meta = this.readMeta(id);
    if (!meta) return;
    meta.status = status; meta.error = error; meta.failedAt = new Date().toISOString();
    this.writeMeta(id, meta);
    this.emit('changed', id);
  }
}
