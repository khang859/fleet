...
export class SupplyRouteService {
  ...
  getDownstream(sectorId: string): SupplyRouteRow[] {
    return this.db
      .prepare<
        [string],
        SupplyRouteRow
      >('SELECT * FROM supply_routes WHERE upstream_sector_id = ? ORDER BY id')
      .all(sectorId);
  }
  ...
  getUpstream(sectorId: string): SupplyRouteRow[] {
    return this.db
      .prepare<
        [string],
        SupplyRouteRow
      >('SELECT * FROM supply_routes WHERE downstream_sector_id = ? ORDER BY id')
      .all(sectorId);
  }
  ...
  getGraph(): Record<string, string[] | undefined> {
    const rows = this.db
      .prepare<
        [],
        { upstream_sector_id: string; downstream_sector_id: string }
