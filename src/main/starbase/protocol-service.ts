...
export class ProtocolService {
  ...
  listProtocols(): ProtocolRow[] {
    return this.db
      .prepare<[], ProtocolRow>('SELECT * FROM protocols ORDER BY built_in DESC, name ASC')
      .all();
  }
  ...
  getExecution(id: string): ProtocolExecutionRow | undefined {
    return this.db
      .prepare<[string], ProtocolExecutionRow>('SELECT * FROM protocol_executions WHERE id = ?')
      .get(id);
  }
  ...
  listExecutions(status?: string): ProtocolExecutionRow[] {
    if (status) {
      return this.db
        ...
        >('SELECT * FROM protocol_executions WHERE status = ? ORDER BY created_at DESC')
        .all(status);
    }
    return this.db
      .prepare<
        [],
        ProtocolExecutionRow
      >('SELECT * FROM protocol_executions ORDER BY created_at DESC')
      .all();
  }
  ...
  getStaleGatePendingExecutions(olderThanSeconds: number): ProtocolExecutionRow[] {
    return this.db
      .prepare<[number], ProtocolExecutionRow>(
