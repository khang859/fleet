...
describe('Hull Gate 2 — verify and lint', () => {
  ...
  function makeOpts(overrides: Partial<HullOpts> = {}): HullOpts {
    const mission = missionSvc.addMission({ sectorId: 'api', summary: 'Test', prompt: 'do something' });
    return {
  ...
  it('verify command pass — continues to PR creation', { timeout: 60_000 }, async () => {
    ...
    const row = db.getDb().prepare('SELECT verify_result, status FROM missions WHERE id = ?').get(opts.missionId) as any;
  ...
  it('verify command fail — sets failed-verification, no PR', { timeout: 60_000 }, async () => {
    ...
    const comms = db.getDb().prepare("SELECT payload FROM comms WHERE from_crew = ? AND type = 'mission_complete'").get(opts.crewId) as any;
    if (comms) {
  ...
  it('verify command timeout — sets failed-verification with timedOut flag', { timeout: 60_000 }, async () => {
    setupWorktreeWithChange();
