...
describe('classifyFromFingerprint', () => {
  ...
  it('returns non-retryable for EACCES', () => {
    expect(classifyFromFingerprint('Error: EACCES: permission denied')).toBe('non-retryable');
  });

  it('returns non-retryable for MODULE_NOT_FOUND', () => {
    expect(classifyFromFingerprint("Error: Cannot find module 'express'")).toBe('non-retryable');
  });

  it('returns non-retryable for 401/403', () => {
    expect(classifyFromFingerprint('HTTP 401 Unauthorized')).toBe('non-retryable');
    expect(classifyFromFingerprint('HTTP 403 Forbidden')).toBe('non-retryable');
  });

  it('returns non-retryable for missing config', () => {
    expect(classifyFromFingerprint('config file not found at /etc/app.json')).toBe('non-retryable');
  });
  ...
  it('returns transient for empty output', () => {
    expect(classifyFromFingerprint('')).toBe('transient');
  });

  it('returns persistent when same fingerprint provided', () => {
    expect(classifyFromFingerprint('some error', 'abc123', 'abc123')).toBe('persistent');
  });

  it('returns transient when different fingerprint provided', () => {
    expect(classifyFromFingerprint('some error', 'abc123', 'def456')).toBe('transient');
  });
});
