...
describe('dispatchTool', () => {
  ...
  it('should add a mission and list it', async () => {
    ...
    require('child_process').execSync('git init && git add . && git commit -m "init" --allow-empty', {
      cwd: sectorDir,
    });
  ...
  it('should resolve a transmission', async () => {
    const id = commsService.send({ from: 'crew-1', to: 'admiral', type: 'hailing', payload: 'Help me' });
    const result = await dispatchTool('resolve', { transmission_id: id, response: 'Here is help' }, deps);
  ...
  it('should deploy via crew service', async () => {
    const result = await dispatchTool(
      'deploy',
      { sector_id: 'my-sector', prompt: 'Fix it' },
      deps,
    );
