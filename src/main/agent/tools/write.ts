...
import type { AgentToolContext, AgentToolResult, WriteArgs } from '../../../shared/agent-tools';
 ...
 * did to it.
 */
export async function runWrite(args: WriteArgs, ctx: AgentToolContext): Promise<AgentToolResult> {
  ...
  if (info === null) {
    ...
    await writeTextFile(ctx.threadId, abs, args.content, false);
  ...
  await writeTextFile(ctx.threadId, abs, args.content, before.crlf);
