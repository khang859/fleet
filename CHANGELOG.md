...
- **Agent status in the sidebar now comes from Claude Code itself** rather than being guessed from what the terminal prints.
  A pane blocked on a permission used to read as busy, and a `(y/n)` in a build log used to read as a question waiting for you.
  Both are now correct, and a pane is handed back to the old behaviour the moment its agent exits (#620).
- **Checkboxes, radios and sliders follow the app theme** - an unchecked box on a dark theme no longer renders as a filled white square that reads as checked (#617).
- **The command palette, quick open, clipboard history, shortcuts, git changes and the other overlays are readable on light themes** - they were dark panels with dark text whatever theme you picked (#590).
- **Sidebar section headings on Fleet Light are legible** - the muted and subtle text now meets the WCAG AA contrast floor instead of reading as disabled controls (#591).
- **`npm run typecheck` no longer leaves the working tree dirty**, so a build no longer shows changes you did not make (#588).
...
- **Images have a home of their own** - every image generated by an agent is now collected in a cross-session Images tab, where it can be opened with its prompt and conversation name, copied, saved, revealed, dragged out, or sent back into a composer as a reference. Gallery attachments are copied into the receiving conversation so they remain available if the original conversation is deleted (#556).
...
- **Installing asks before it stops running agents** - `Restart to Update` went straight to the updater, which on Windows and Linux spawns the installer _before_ it asks the app to quit. The close warning added in the last release then appeared after the installer was already running, so answering 'cancel' left the app open and being replaced underneath it. The question is now asked first, through the same check a Cmd+Q goes through, and nothing is spawned until it has been answered.
